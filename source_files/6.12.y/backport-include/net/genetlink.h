...
#if LINUX_VERSION_IS_EQU(5,14,0) && RHEL_RELEASE_IS_GEQ(9,5)
/* RHEL backported the genl_info_userhdr */
...
#elif LINUX_VERSION_IS_LESS(6,11,0) && \
...
int backport_genlmsg_multicast_allns(const struct genl_family *family,
				     ...
				     unsigned int group)
{
	return genlmsg_multicast_allns(family, skb, portid, group, GFP_ATOMIC);
}
#define genlmsg_multicast_allns LINUX_BACKPORT(genlmsg_multicast_allns)

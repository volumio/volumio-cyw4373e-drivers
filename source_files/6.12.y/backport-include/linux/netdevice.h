#ifndef __BACKPORT_LINUX_NETDEVICE_H
#define __BACKPORT_LINUX_NETDEVICE_H
...
/* new kernels include <net/netprio_cgroup.h> which has this */
#include <linux/hardirq.h>
/* new kernels include <linux/bpf.h> which has this */
...
backport_alloc_netdev_mq(int sizeof_priv, const char *name,
				    unsigned char name_assign_type,
				    void (*setup)(struct net_device *),
				    unsigned int queue_count)
{
	return alloc_netdev_mq(sizeof_priv, name, setup, queue_count);
}
#define alloc_netdev_mq LINUX_BACKPORT(alloc_netdev_mq)

#define alloc_netdev_mqs(sizeof_priv, name, name_assign_type, setup, txqs, rxqs) \
	alloc_netdev_mq(sizeof_priv, name, name_assign_type, setup, \
			max_t(unsigned int, txqs, rxqs))

#undef alloc_netdev
#define alloc_netdev(sizeof_priv, name, name_assign_type, setup) \
	alloc_netdev_mq(sizeof_priv, name, name_assign_type, setup, 1)

#endif /* LINUX_VERSION_IS_GEQ(2,6,38) */
...
#define skb_csum_hwoffload_help(skb, features) skb_checksum_help(skb)
#endif

#if LINUX_VERSION_IN_RANGE(3,9,0, 4,15,0)
...
static inline int backport_netif_rx(struct sk_buff *skb)
{
...
static inline void
backport_netif_napi_add(struct net_device *dev, struct napi_struct *napi,
	       int (*poll)(struct napi_struct *, int))
{
...
#endif /* __BACKPORT_LINUX_NETDEVICE_H */

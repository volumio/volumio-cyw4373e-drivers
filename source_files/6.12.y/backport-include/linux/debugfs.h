...
static inline
struct dentry *backport_debugfs_create_file(const char *name, mode_t mode,
...
#define debugfs_create_file LINUX_BACKPORT(debugfs_create_file)

static inline
struct dentry *backport_debugfs_create_dir(const char *name, struct dentry *parent)
{
	if (IS_ERR(parent))
		return parent;
...
#define debugfs_create_dir LINUX_BACKPORT(debugfs_create_dir)

static inline
struct dentry *backport_debugfs_create_symlink(const char *name, struct dentry *parent,
				      const char *dest)
{
	if (IS_ERR(parent))
		return parent;

	return debugfs_create_symlink(name, parent, dest);
}
#define debugfs_create_symlink LINUX_BACKPORT(debugfs_create_symlink)

...
#else
	int ret = of_property_read_u8_array(dev_of_node(dev), name, addr, alen);
#endif
...
void *device_get_mac_address(struct device *dev, char *addr)
{
	...
	res = device_get_mac_addr(dev, "mac-address", addr, ETH_ALEN);
	if (res)
		return res;

	res = device_get_mac_addr(dev, "local-mac-address", addr, ETH_ALEN);
	if (res)
		return res;

	return device_get_mac_addr(dev, "address", addr, ETH_ALEN);
}

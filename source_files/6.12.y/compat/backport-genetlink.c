...
#elif LINUX_VERSION_IS_GEQ(3,7,0)
		NETLINK_CB(in_skb).ssk->sk_err = ENOBUFS;
		sk_error_report(NETLINK_CB(in_skb).ssk);
#endif
		return;
	}
 ...
 */
	...
	for (i = 0; i < family->n_ops; i++) {
		void *opsn = ops + i;
		err = genl_register_ops(&family->family, opsn);
		if (err < 0)
			goto error;
	}
				     ...
				     unsigned int group)
{
	...
	if (group == INVALID_GROUP)
		return -EINVAL;
	return genlmsg_mcast(skb, portid, group, GFP_ATOMIC);
}

...
#else
		status = tty->driver->ops->tiocmset(tty, NULL, set, clear);
#endif
...
#elif LINUX_VERSION_IS_GEQ(5,14,0)
 ...
 */
	...
	(LINUX_VERSION_IS_EQU(5,14,0) && RHEL_RELEASE_IS_GEQ(9,1))
...
#endif
{
	...
	(LINUX_VERSION_IS_EQU(5,14,0) && RHEL_RELEASE_IS_GEQ(9,3))
...
#if LINUX_VERSION_IS_GEQ(5,10,20)
...
static long hci_uart_tty_compat_ioctl(struct tty_struct *tty, struct file *file,
			      unsigned int cmd, unsigned long arg)
{
	return hci_uart_tty_ioctl(tty, file, cmd, arg);
}
#endif
...
static void __exit hci_uart_exit(void)
{
	...
	int err = tty_unregister_ldisc(N_HCI);
	if (err)

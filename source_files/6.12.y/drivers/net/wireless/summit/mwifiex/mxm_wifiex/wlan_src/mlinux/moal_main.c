...
static mlan_status woal_add_card_dpc(moal_handle *handle)
{
	...
	if (handle->params.httxcfg_2) {
		moal_private *priv = NULL;
		uint16_t drv_mode_local = handle->params.httxcfg_2 >> 16;
		bool bSet = false;
		...
		for (i = 0; i < MIN(handle->priv_num, MLAN_MAX_BSS_NUM); i++) {
			...
			#ifdef STA_SUPPORT
			if ((drv_mode_local & DRV_MODE_STA) && priv->bss_type == MLAN_BSS_TYPE_STA) {
				bSet = true;
			}
			#endif
			#ifdef UAP_SUPPORT
			if ((drv_mode_local & DRV_MODE_UAP) && priv->bss_type == MLAN_BSS_TYPE_UAP) {
				bSet = true;
			}
			#endif
			#ifdef WIFIDIRECT_SUPPORT
			if ((drv_mode_local & DRV_MODE_WIFIDIRECT) && priv->bss_type == MLAN_BSS_TYPE_WIFIDIRECT) {
				bSet = true;
			}
			#endif

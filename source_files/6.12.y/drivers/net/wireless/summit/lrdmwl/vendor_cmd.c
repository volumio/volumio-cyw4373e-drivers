...
static int
lrd_vendor_cmd_get_channel(struct wiphy *wiphy, struct wireless_dev *wdev,
			       const void *data, int data_len)
{
	struct ieee80211_hw     *hw = wiphy_to_ieee80211_hw(wiphy);
	struct mwl_priv       *priv = hw->priv;
	...
	msg = cfg80211_vendor_cmd_alloc_reply_skb(wiphy,sizeof(u32) + sizeof(u16));

	if (msg) {
		nla_put_u32(msg, LRD_ATTR_CMD_RSP, 0);
		nla_put(msg, LRD_ATTR_DATA, sizeof(u16), (const void *)&priv->curr_channel);
		rc = cfg80211_vendor_cmd_reply(msg);
	}
	else {
		rc = -ENOMEM;
			       ...
			       const void *data, int data_len)
{
	...
	if (unlikely(!skb)) {
		ret = -ENOMEM;
		wiphy_err(hw->wiphy,"skb alloc failed\n");
		goto done;
	}

	nla_put_u32(skb, ATTR_RID_FAMILY, RID_ATTR_FAMILY_NXP);
	nla_put_u32(skb, ATTR_RID_TYPE, RID_ATTR_TYPE_60 );
	switch (priv->host_if) {
		case MWL_IF_SDIO:
			nla_put_u32(skb, ATTR_RID_WIFI_BUS, RID_ATTR_BUS_SDIO);
		break;
		case MWL_IF_USB:
			nla_put_u32(skb, ATTR_RID_WIFI_BUS, RID_ATTR_BUS_USB);
		break;
		case MWL_IF_PCIE:
			nla_put_u32(skb, ATTR_RID_WIFI_BUS, RID_ATTR_BUS_PCIE);
		break;
		default:
		break;
	}
	...
	if (unlikely(ret))
		wiphy_err(hw->wiphy, "Vendor Command reply failed ret:%d\n", ret);
...
static const struct wiphy_vendor_command lrd_vendor_commands[] = {
...

	{
		.info = {
			.vendor_id = LRD_OUI,
			.subcmd    = LRD_VENDOR_CMD_GET_CHANNEL,
		},
		.flags = 0,
		.doit  = lrd_vendor_cmd_get_channel,
		.policy = VENDOR_CMD_RAW_DATA,
	},

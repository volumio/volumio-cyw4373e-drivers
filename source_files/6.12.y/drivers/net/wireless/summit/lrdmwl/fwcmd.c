...
static int is_host_cipher(const u8 *buf)
{
	u32 cipher;
...
static int ap_select_host_crypto(struct mwl_vif *mwl_vif)
{
	const u8 *rsn_ie;
	size_t rsn_ie_len;
	int cnt;
				   ...
				   struct mwl_vif *vif, u8 *beacon, int len)
{
	...
	vif->force_host_crypto = ap_select_host_crypto(vif);
}
				   ...
				   struct ieee80211_vif *vif)
{
	...
	phy_ds_param_set->current_chnl = vif->bss_conf.chanctx_conf->def.chan->hw_value;
				      ...
				      struct ieee80211_bss_conf *bss_conf)
{
	...
	u8 chnl_len;
	bool a_band;
	bool enable = !!b_inf->ie_country_ptr;

	if (enable) {
		chnl_len = b_inf->ie_country_len - 5;
		switch (bss_conf->chanctx_conf->def.chan->band) {
...
int mwl_fwcmd_set_cfg_data(struct ieee80211_hw *hw, u16 type)
{
	...
	pcmd->cmd_hdr.len = cpu_to_le16(sizeof(*pcmd) +	le16_to_cpu(pcmd->data_len));
	pcmd->action = cpu_to_le16(HOSTCMD_ACT_GEN_SET);
...
static int mwl_fwcmd_set_roc_channel(struct ieee80211_hw *hw,
					struct ieee80211_channel *channel)
{
	...
	mutex_unlock(&priv->fwcmd_mutex);
	priv->curr_channel = channel->hw_value;
	return 0;
}
				 ...
				 struct ieee80211_key_conf *key)
{
	...
	pcmd2->action_data = ENCR_TYPE_WEP;

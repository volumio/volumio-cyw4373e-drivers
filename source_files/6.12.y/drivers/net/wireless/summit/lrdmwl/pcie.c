...
#define LRD_PCIE_DESC        "Ezurio 60 Series Wireless PCIE Network Driver"
...
static int mwl_alloc_pci_resource(struct mwl_priv *priv)
{
	...
	priv->pcmd_buf = dmam_alloc_coherent(priv->dev,
						      CMD_BUF_SIZE,
						      &priv->pphys_cmd_buf,
						      GFP_KERNEL);
	if (!priv->pcmd_buf) {
	...
	wiphy_debug(priv->hw->wiphy,
		    "priv->pcmd_buf = %px  priv->pphys_cmd_buf = %pad\n",
...
static int mwl_rx_ring_init(struct mwl_priv *priv)
{
	...
	if (desc->prx_ring) {
		...
		for (i = 0; i < SYSADPT_MAX_NUM_RX_DESC; i++) {
			...
			// dma_set_dma_mask() in mwl_pcie_init()
			...
			val = lower_32_bits(desc->pphys_rx_ring) +
		...
		desc->prx_ring[SYSADPT_MAX_NUM_RX_DESC - 1].pphys_next =
			cpu_to_le32(lower_32_bits(desc->pphys_rx_ring));
...
static int mwl_tx_ring_alloc(struct mwl_priv *priv)
{
	...
	memset(desc->ptx_ring, 0x00, MAX_NUM_TX_RING_BYTES * SYSADPT_NUM_OF_DESC_DATA);
	...
	for (num = 0; num < SYSADPT_NUM_OF_DESC_DATA; num++) {
		...
		desc->pphys_tx_ring = priv->desc_data[0].pphys_tx_ring +
			num * MAX_NUM_TX_RING_BYTES;
	}

	mem = kzalloc(MAX_NUM_TX_HNDL_BYTES * SYSADPT_NUM_OF_DESC_DATA,
		      GFP_KERNEL);
		...
		if (!IS_PFU_ENABLED(priv->chip_type)) {
			...
			if (desc->ptx_ring) {
				...
				desc->ptx_ring[i].status =
					...
					cpu_to_le32(lower_32_bits(desc->pphys_tx_ring) +
...
static int mwl_pcie_init_post(struct mwl_priv *priv)
{
	...
	if (!IS_PFU_ENABLED(priv->chip_type)) {
		for (i = 0; i < SYSADPT_TOTAL_TX_QUEUES; i++)
			writel(lower_32_bits(priv->desc_data[i].pphys_tx_ring),
				card->iobase0 + priv->desc_data[i].wcb_base);
	}

	writel(lower_32_bits(priv->desc_data[0].pphys_rx_ring),
...
static void mwl_pcicmd(struct mwl_priv *priv)
{
	struct mwl_pcie_card *card = priv->intf;
	 ...
	 */
	writel(lower_32_bits(priv->pphys_cmd_buf), card->iobase1 + MACREG_REG_GEN_PTR);

	/* Write the upper 32bits of the physical address to high command
	 * address scratch register
	 */
	writel(upper_32_bits(priv->pphys_cmd_buf), card->iobase1 + MACREG_REG_INT_CODE);
}
...
static int mwl_pcie_program_firmware(struct mwl_priv *priv)
{
	...
	/* Now firmware is downloaded successfully, so this part is to check
...
static int mwl_pcie_send_command(struct mwl_priv *priv)
{
	...
	if (priv->mfg_mode) {
		...
		writel(lower_32_bits(priv->pphys_cmd_buf), card->iobase1 + MACREG_REG_CMDRSP_BUF_LO);
		writel(upper_32_bits(priv->pphys_cmd_buf), card->iobase1 + MACREG_REG_CMDRSP_BUF_HI);
	}
...
static void mwl_non_pfu_tx_done(unsigned long data)
{
	struct ieee80211_hw *hw = (struct ieee80211_hw *)data;
	struct mwl_priv *priv = hw->priv;
	struct mwl_pcie_card *card = priv->intf;
	...
	for (num = 0; num < SYSADPT_TX_WMM_QUEUES; num++) {
		...
		while (tx_hndl &&
			...
			if (done_skb) {
				...
				ieee80211_tx_status_skb(hw, done_skb);
			}
...
static int mwl_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	card = devm_kzalloc(&pdev->dev, sizeof(struct mwl_pcie_card), GFP_KERNEL);
	if (!card)
		return -ENOMEM;

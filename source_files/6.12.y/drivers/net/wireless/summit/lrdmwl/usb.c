...
#define LRD_USB_DESC        "Summit 60 Series Wireless USB Network Driver"
...
static void mwl_usb_reset(struct usb_card_rec *card)
{
			     ...
			     const struct usb_device_id *id)
{
	...
	struct device_node *of_node = dev_of_node(&intf->dev);;
	...
	if (of_node && !of_match_node(mwl_usb_of_match_table, of_node))
		of_node = NULL;

	/* device tree node parsing and platform specific configuration */
	rc = mwl_gpio_init(&intf->dev, of_node, &card->reset_pwd);
	if (rc) {
		if (rc != -EPROBE_DEFER)
			dev_err(&udev->dev, "PMU_EN GPIO failed: %d\n", rc);
	...
	if (mwl_gpio_valid(card->reset_pwd)) {
		...
		dev_info(&udev->dev, "PMU_EN GPIO %d configured\n", mwl_gpio_num(card->reset_pwd));
	}
	else
		dev_info(&udev->dev, "PMU_EN GPIO not configured\n");
...
*/
static void mwl_handle_rx_packet(struct mwl_priv *priv, struct sk_buff *skb)
{
...
static void  mwl_usb_cleanup(struct mwl_priv *priv)
{
...
static int __init mwl_usb_driver_init(void)
{
	int rc;
	...
	rc = usb_register(&mwl_usb_driver);
	if (rc)
...
static void __exit mwl_usb_driver_exit(void)
{
	usb_deregister(&mwl_usb_driver);

	mwl_gpio_release_legacy();
}

module_init(mwl_usb_driver_init);
module_exit(mwl_usb_driver_exit);

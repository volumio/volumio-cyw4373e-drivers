...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,11,0)) || defined(WITH_BACKPORTS)
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 9, 0)) || defined(WITH_BACKPORTS)
    cfg80211_ch_switch_notify(dev, &chandef, 0);
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 3, 0)) || defined(FORCE_MLO_SUPPORT)
...
static void bp_net_ops_struct_hdd_softap_tx_timeout(struct net_device *dev){
    hdd_softap_tx_timeout(dev, 0);
}
#endif
...
#else
    .ndo_tx_timeout = bp_net_ops_struct_hdd_softap_tx_timeout,
#endif
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0))
        dev_addr_mod(pWlanHostapdDev, 0, macAddr, sizeof(tSirMacAddr));
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 12, 0)) || defined(WITH_BACKPORTS)

...
static int pktlog_enable_buf, pktlog_size_buf;
		...
		ppos)
{
	...
	if (write) {
		...
		pktlog_size_buf = get_pktlog_bufsize(pl_dev);
		ret = ATH_SYSCTL_PROC_DOINTVEC(ctl, write, filp, buffer,
					       lenp, ppos);
	}
...
#define set_ctl_name(a, b)	pl_info_lnx->sysctls[a].ctl_name = b
	...
	pl_info_lnx->sysctls[4].data = &pktlog_enable_buf;
	pl_info_lnx->sysctls[4].maxlen = sizeof(pktlog_enable_buf);
	...
	pl_info_lnx->sysctls[5].data = &pktlog_size_buf;
	pl_info_lnx->sysctls[5].maxlen = sizeof(pktlog_size_buf);
	...
	{
		char path[128];
		snprintf(path, sizeof(path), PKTLOG_PROC_DIR "/%s", proc_name);
		pl_info_lnx->sysctl_header =
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6,6,0))
			register_sysctl_sz(path, pl_info_lnx->sysctls + 4, 8);
#else
			register_sysctl(path, pl_info_lnx->sysctls + 4);
#endif
	}

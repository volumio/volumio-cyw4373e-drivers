...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 15, 0)) || defined(WITH_BACKPORTS)
...
#if (LINUX_VERSION_CODE > KERNEL_VERSION(5,5,0)) || defined(WITH_BACKPORTS)
...
static void bp_epping_drv_ops_epping_tx_queue_timeout(struct net_device *dev){
   epping_tx_queue_timeout(dev, 0);
}
#endif
...
#else
   .ndo_tx_timeout = bp_epping_drv_ops_epping_tx_queue_timeout,
#endif
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0))
   dev_addr_mod(dev, 0, macAddr, sizeof(tSirMacAddr));
#else

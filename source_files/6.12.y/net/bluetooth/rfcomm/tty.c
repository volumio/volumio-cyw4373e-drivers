...
#endif
{
	...
	BT_DBG("tty %p count %d", tty, (int)count);
...
#else
static int rfcomm_tty_ioctl(struct tty_struct *tty, struct file * file, unsigned int cmd, unsigned long arg)
#endif
{
...
#else
static int rfcomm_tty_tiocmget(struct tty_struct *tty, struct file *file)
#endif
{
	struct rfcomm_dev *dev = tty->driver_data;
...
#else
static int rfcomm_tty_tiocmset(struct tty_struct *tty, struct file *file, unsigned int set, unsigned int clear)
#endif
{
	struct rfcomm_dev *dev = tty->driver_data;
	struct rfcomm_dlc *dlc = dev->dlc;
